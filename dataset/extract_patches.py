...
if __name__ == '__main__':
    ...
    parser.add_argument('-slide_dir', '--slide_dir', type=str, default="",help='Raw slide dir')
    parser.add_argument('-tile_dir', '--tile_dir', type=str, default="",help='Tiled patches dir')
    parser.add_argument('-base_dir', '--base_dir', type=str, default="",help='Tiled patches dir')
    ...
    parser.add_argument('-t', '--background_t', type=int, default=20, help='Threshold for filtering background [15]')  
    args = parser.parse_args()
    levels = tuple(sorted(args.magnifications))
    assert len(levels)<=2, 'Only 1 or 2 magnifications are supported!'
    path_base = os.path.join(args.base_dir, args.dataset,args.slide_dir)
    print(path_base)
    all_slides = glob.glob(path_base+f"/*/*.{args.slide_format}")
    print(all_slides) 
    
    # pos-i_pos-j -> x, y
    for idx, c_slide in enumerate(all_slides):
         print('Process {} slide {}/{}'.format(args.dataset,idx+1, len(all_slides)))
         name=os.path.basename(c_slide)[:-4]
         if os.path.exists(os.path.join(args.base_dir,args.dataset,args.tile_dir,name)):
             continue
         DeepZoomStaticTiler(c_slide, os.path.join(args.base_dir,args.dataset,args.tile_dir,name), levels, args.base_mag, args.objective, args.format, args.tile_size, args.overlap, True, args.quality, args.workers, args.background_t).run()
