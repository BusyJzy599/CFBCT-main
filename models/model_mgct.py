...
class MGCT_Layer(nn.Module):
    ...
    def forward(self, wsi_feat_embed, omic_feat_embed, return_attn=False):
        ...
        if self.use_trans:
            omic_wsi_embed = self.alpha_trans(omic_wsi_embed.unsqueeze(0))
        if self.use_ffn:
        ...
        if self.use_trans:
            wsi_omic_embed = self.beta_trans(wsi_omic_embed.unsqueeze(0))
        if self.use_ffn:
        ...
        if self.fusion == 'concat':
            ...
            fused_embed = self.gamma(torch.cat([omic_wsi_embed.squeeze(0), wsi_omic_embed.squeeze(0)], axis=1))
...
class MGCT_Surv(nn.Module):
    def __init__(self, fusion='concat', stage1_num_layers=1, stage2_num_layers=2,
        ...
        self.size_dict_WSI = {"small": [768, 256, 256], "big": [768, 256, 256]}
    ...
    def forward(self, **kwargs):
        ...
        wsi_feats = kwargs['x_path'].squeeze(1)
        ...
        for stage2_layer in self.stage2_layers:
            ...
            if self.return_attn:
                ...
                stage2_output = stage2_layer(stage1_output.unsqueeze(0), wsi_feats_embed, return_attn=False)
