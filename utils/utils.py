...
def caculate_risk(logits):
    Y_hat = torch.topk(logits, 1, dim = 1)[1]
    hazards = torch.sigmoid(logits)
    S = torch.cumprod(1 - hazards, dim=1)
    rsk=-torch.sum(S, dim=1).detach().cpu().numpy()
    return hazards,rsk
...
def temperature_scaled_softmax(logits, temperature=0.1):
...
def collate_MIL_survival_sig(batch):
    img = torch.cat([item[0] for item in batch], dim = 0)
    omic1 = [item[1] for item in batch]
   
    label = torch.LongTensor([item[2] for item in batch])
    event_time = np.array([item[3] for item in batch])
    c = torch.FloatTensor([item[4] for item in batch])
    return [img, omic1, label, event_time, c]


def collate_MIL_survival_mb(batch):
    img = torch.cat([item[0] for item in batch], dim = 0)
    omics=[item[1] for item in batch]
    label = torch.LongTensor([item[2] for item in batch])
    event_time = np.array([item[3] for item in batch])
    c = torch.FloatTensor([item[4] for item in batch])
    return [img, omics, label, event_time, c]
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='coattn', batch_size=1):
    ...
    if mode == 'coattn':
        ...
        collate= collate_MIL_survival_mb
    elif mode == 'cluster':
...
def get_train_loader(split_dataset, mode='coattn', batch_size=1):
    """
        return either the validation loader or training loader 
    """
    if mode == 'coattn':
...
def nll_loss(hazards, S, Y, c, alpha=0.4, eps=1e-7,epoch=1):
    batch_size = len(Y)
    ...
    #h[y] = h(1)
    #S[1] = S(1)
    p1=torch.log(torch.gather(S_padded, 1, Y).clamp(min=eps))
    p2=torch.log(torch.gather(hazards, 1, Y).clamp(min=eps))
    p3=torch.log(torch.gather(S_padded, 1, Y+1).clamp(min=eps))

    uncensored_loss = -(1 - c) * (p1 +p2 )
    censored_loss = - c * p3
...
def l1_reg_cat(model, reg_type=None):
    l1_reg = 0

    l1_reg += l1_reg_all(model.sig_networks)
    l1_reg += l1_reg_all(model.mm)
...
def get_custom_exp_code(args):
    ...
    if args.model_type == 'snn':
      ...
      param_code += f'CFBCT-{args.W_k}-{args.ema}-{args.tmp}-{args.b}b'
...
class Writer(object):
    def __init__(self,
        ...
        super().__init__()
        self.writer_file_path_base = os.path.join(writter_path,config.data.name, 'writer')
        self.writer_file_path = os.path.join(self.writer_file_path_base,file_name) 
        if not os.path.exists(self.writer_file_path_base): os.makedirs(self.writer_file_path_base)
        self.writer=SummaryWriter(logdir=self.writer_file_path ,flush_secs=5)

    def add_linear(self,tag,scores,global_step):
        self.writer.add_scalar(tag, scores, global_step)
    def add_linears(self,tag,scores,global_step):
        self.writer.add_scalars(tag, scores, global_step)
...
class Logger(object):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.logger_file_path_base = logger_path
        self.logger_file_path = os.path.join( logger_path,file_name+".log") 


        self.logger = logging.getLogger(logger_name)
        self.logger.setLevel(Logger._Levels[level])
        if not os.path.exists(self.logger_file_path_base): os.makedirs(self.logger_file_path_base)

        self.file_handler = logging.FileHandler(self.logger_file_path,'w')
        self.file_handler.setLevel(Logger._Levels[level])

        self.console_handler = logging.StreamHandler()
        self.console_handler.setLevel(Logger._Levels[level])
...
def format_time(run_time):
    ...
    return f'Time spent:{hour}h {minute}m {second}s'
