...
def train_loop_survival_coattn_mt(epoch, bs_micro, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=32, args=None):
    model.train()
    ...
    for batch_idx, (data_WSI, data_omic, label, event_time, c) in enumerate(loader):
        ...
        for tindex in index_chunk_list:
            ...
            output= model(x_path=wsi_mb, x_omic1=data_omic1, x_omic2=data_omic2, x_omic3=data_omic3, x_omic4=data_omic4, x_omic5=data_omic5, x_omic6=data_omic6)
            if args.bag_loss == 'nll_surv':
                loss_micro = loss_fn['surv_loss'](hazards=output['hazards'], S=output['S'], Y=label, c=c)
            else:
                raise NotImplementedError
            loss += loss_micro
            all_risk += -torch.sum(output['S'], dim=1).detach().cpu().numpy().item()
            cnt+=1
...
def validate_survival_coattn_mt(cur, epoch, bs_micro, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None, args=None):
    model.eval()
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu") 
