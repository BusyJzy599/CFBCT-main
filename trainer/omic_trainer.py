...
def train_survival(epoch, model, loader, optimizer,  writer=None, loss_fn=None, reg_fn=None, args=None):   
    ...
    for batch_idx, (_, data_omic, label, event_time, c) in enumerate(loader):

        label = label.type(torch.LongTensor).to(device)
        c = c.type(torch.FloatTensor).to(device)

        output = model(x_omic=data_omic.type(torch.FloatTensor).to(device))
...
def validate_survival( epoch, model, loader,  writer=None, loss_fn=None, reg_fn=None, args=None):
    model.eval()
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu") 
